...
<div class="btn_confirm">
    <input type="submit" value="검색" class="btn_medium" id="searchBtn">
</div>
</form>
...
<div class="tbl_head01">
	...
	<tr>
		...
		<th scope="col"><a href="/admin/member.php?code=list&page=&filed=grade&orderby=asc">레벨</a></th>
...
<%
	...
	<%
		...
		String grdNo = (String) session.getAttribute("grdNo");
...
// 		alert( $(this).text() );
		location.href="/BeautyInSite/listSearch.do?cate=<%=cate%>&grdNo=<%=grdNo%>&search=<%=search%>&page=" + $(this).text();
	});
...
// 		alert( parseInt($('.pagination').children().last().text()) +1 );
		...
		location.href= "/BeautyInSite/listSearch.do?cate=<%=cate%>&grdNo=<%=grdNo%>&search=<%=search%>&page=" + currentPage;
	});
	...
	$('.prev').on('click',function(){
		...
		location.href="/BeautyInSite/listSearch.do?page=" + currentPage;
	});
