package com.cosmetic.member.account.controller;

import java.io.IOException;
import java.util.HashMap;
import java.util.Map;

import javax.servlet.ServletException;
import javax.servlet.annotation.MultipartConfig;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import javax.servlet.http.Part;

import org.apache.commons.beanutils.BeanUtils;

import com.cosmetic.member.account.service.IMemberService;
import com.cosmetic.member.account.service.ISkinService;
import com.cosmetic.member.account.service.MemberServiceImpl;
import com.cosmetic.member.account.service.SkinServiceImpl;
import com.cosmetic.member.account.vo.AccountVO;
import com.cosmetic.member.account.vo.SkinVO;
import com.cosmetic.util.Const;
import com.cosmetic.util.OthersUtil;

@MultipartConfig
@WebServlet("/register.do")
public class AccountRegisterController extends HttpServlet {
	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		String url = "/main/account/register.jsp";
		req.getRequestDispatcher(url).forward(req, resp);
	}

	@Override
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		IMemberService memService = MemberServiceImpl.getInstance();

		if (req.getSession().getAttribute("naverProfile") != null) {
			req.getSession().removeAttribute("naverProfile");
			req.getSession().removeAttribute("result");
		}
		AccountVO accVO = AccountVO.builder().build();

		try {
			BeanUtils.populate(accVO, req.getParameterMap());
			accVO.setPw(OthersUtil.SHA256Encryption(accVO.getPw()));
			Part part = req.getPart("profileImg");
			if (part.getSubmittedFileName() != null && !part.getSubmittedFileName().equals(""))
				accVO.setImgProfile(OthersUtil.Base64Encode(part));
			else
				accVO.setImgProfile(
						"");
		} catch (Exception e) {
			e.printStackTrace();
		}

		String naverId = req.getParameter("naverId");
		String kakaoId = req.getParameter("kakaoId");

		String msg = "";
		String[] troubles = req.getParameterValues("trouble");
		if (troubles != null) {
			SkinVO skVO = SkinVO.builder().build();
			skVO.setSkType(req.getParameter("skType"));

			ISkinService skService = SkinServiceImpl.getInstance();
			int skNo = skService.insertSkinType(skVO, troubles);

			if (skNo > 0) {
				accVO.setSkNo(skVO.getSkNo());
				accVO.setAccNo(skVO.getAccNo());
				int register = memService.registerMember(accVO);

				if (register > 0) {
					msg = Const.SUCCESS.getVal();
				} else {
					msg = Const.FAILURE.getVal();
				}
			}

		}

		if (naverId != null && !naverId.equals("")) {
			Map<String, Object> naver = new HashMap<>();
			naver.put("accNo", accVO.getAccNo());
			naver.put("authNaverid", naverId);
			memService.registerAuth(naver);

		} else if (kakaoId != null && !kakaoId.equals("")) {
			Map<String, Object> kakao = new HashMap<>();
			kakao.put("accNo", accVO.getAccNo());
			kakao.put("authKakaoid", kakaoId);
			memService.registerAuth(kakao);
		}

		req.getSession().setAttribute("msg", msg);

		String url = "/index.do";
		resp.sendRedirect(req.getContextPath() + url);
	}
}
